/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from './routes/__root'
import { Route as IndexRouteImport } from './routes/index'
import { Route as PlayerSteamIDRouteImport } from './routes/player/$steamID'

const IndexRoute = IndexRouteImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRouteImport,
} as any)
const PlayerSteamIDRoute = PlayerSteamIDRouteImport.update({
  id: '/player/$steamID',
  path: '/player/$steamID',
  getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
  '/': typeof IndexRoute
  '/player/$steamID': typeof PlayerSteamIDRoute
}
export interface FileRoutesByTo {
  '/': typeof IndexRoute
  '/player/$steamID': typeof PlayerSteamIDRoute
}
export interface FileRoutesById {
  __root__: typeof rootRouteImport
  '/': typeof IndexRoute
  '/player/$steamID': typeof PlayerSteamIDRoute
}
export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths: '/' | '/player/$steamID'
  fileRoutesByTo: FileRoutesByTo
  to: '/' | '/player/$steamID'
  id: '__root__' | '/' | '/player/$steamID'
  fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
  IndexRoute: typeof IndexRoute
  PlayerSteamIDRoute: typeof PlayerSteamIDRoute
}

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/player/$steamID': {
      id: '/player/$steamID'
      path: '/player/$steamID'
      fullPath: '/player/$steamID'
      preLoaderRoute: typeof PlayerSteamIDRouteImport
      parentRoute: typeof rootRouteImport
    }
  }
}

const rootRouteChildren: RootRouteChildren = {
  IndexRoute: IndexRoute,
  PlayerSteamIDRoute: PlayerSteamIDRoute,
}
export const routeTree = rootRouteImport
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()
